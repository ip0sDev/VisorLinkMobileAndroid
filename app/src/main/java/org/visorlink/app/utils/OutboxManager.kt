            ...
            when (action.type) {
                ...
                "image" -> {
                    ...
                    if (file.exists()) {
                        ...
                        val uploadResult = googleDriveMediaService.uploadMediaFile(token, folderId, file, "image/jpeg") { progress ->
                ...
                "voice" -> {
                    ...
                    if (file.exists()) {
                        ...
                        val uploadResult = googleDriveMediaService.uploadMediaFile(token, folderId, file, "audio/webm") { progress ->
                            scope.launch { updateProgressThrottled(action.id, progress) }
                        }
                ...
                "audio" -> {
                    ...
                    if (file.exists()) {
                        ...
                        if (coverLocalPath.isNotBlank()) {
                            ...
                            if (coverFile.exists()) {
                                try {
                                    val coverResult = googleDriveMediaService.uploadMediaFile(token, folderId, coverFile, "image/jpeg") {}
                                    coverMediaId = coverResult.fileId
                                    coverFile.delete()
                                } catch (_: Exception) {}
                            }
                        }
                        val uploadResult = googleDriveMediaService.uploadMediaFile(token, folderId, file, "audio/mpeg") { progress ->
                            scope.launch { updateProgressThrottled(action.id, progress) }
                        }
