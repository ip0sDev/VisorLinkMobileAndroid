...
- [x] **Валидация привязки Telegram.** Cloud Function `generateTgCode` возвращает код, но процесс завершения привязки происходит на стороне бота. Стоит добавить слушатель изменений в профиле пользователя, чтобы экран настроек обновлялся мгновенно, когда бот запишет `tg_uid`.
- [x] **Оптимизация Timeline.** Сейчас 24-часовой график пересчитывается при каждом обновлении списка инцидентов. Для списка из 100+ элементов это может быть накладно, стоит кэшировать результат.

## Результаты ревизии безопасности и архитектуры

- [x] **Утечка памяти и некорректная архитектура в MainActivity.** Заменить самописный `CoroutineScope` на `lifecycleScope`. Убрать неиспользуемые переменные. Исправить вызов `verifyOfficialClient` (использовался неверный регион `Firebase.functions` по умолчанию, а не `europe-west1` через DI).
- [x] **Использование Context во ViewModels.** Многие ViewModel (ThemeViewModel, ChatViewModel и др.) получают `Context` напрямую. Хотя DI передает Application Context, это концептуальное нарушение MVVM. Желательно вынести логику работы с SharedPreferences в слой Repository (SettingsRepository) и перевести остальные ViewModel на AndroidViewModel.
- [x] **Хранение чувствительных данных в открытом виде.** `StealthManager` (режим инкогнито) и `DraftManager` (черновики сообщений) используют обычные `SharedPreferences`. Их следует перевести на `EncryptedSharedPreferences` (как в `TfaManager`), чтобы исключить компрометацию данных при физическом доступе к ФС (например, на рутованных устройствах).
- [x] **Безопасность токенов и бэкенда.** Убедиться, что `DynamicBaseUrlInterceptor` и настройки кастомного бэкенда не позволяют осуществлять атаки типа SSRF или перенаправление токенов авторизации на сторонние серверы (сейчас `http://10.0.2.2:8080` может быть подменен через настройки).
